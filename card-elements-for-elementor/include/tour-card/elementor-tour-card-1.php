<?php
/**
 * Tour Card 1 - Elementor widget template.
 *
		...
		<div class="tour-container">
			<div class="tour-content">
				...
				if ( ! empty( $settings['tour_sale_icon']['value'] ) ) {
					ob_start();
					Icons_Manager::render_icon( $settings['tour_sale_icon'], array( 'aria-hidden' => 'true' ) );
					$sale_icon = ob_get_clean();
				}
				if ( 'yes' === $settings['tour_sale'] ) {
					?>
			...
			<div class="tour-details">
				<ul class="tour-detail-ul">
					...
					if ( $settings['tour_days_icon']['value'] ) {
						ob_start();
						Icons_Manager::render_icon( $settings['tour_days_icon'], array( 'aria-hidden' => 'true' ) );
						$icon1 = ob_get_clean();
					}
					?>
					...
					if ( $settings['tour_person_icon']['value'] ) {
						ob_start();
						Icons_Manager::render_icon( $settings['tour_person_icon'], array( 'aria-hidden' => 'true' ) );
						$icon2 = ob_get_clean();
					}
					?>
					...
					if ( $settings['tour_guide_icon']['value'] ) {
						ob_start();
						Icons_Manager::render_icon( $settings['tour_guide_icon'], array( 'aria-hidden' => 'true' ) );
						$icon3 = ob_get_clean();
					}
					?>
			...
			<div class="tour-button">
				<a href="<?php echo esc_url( $tour_btn_link['url'] ); ?>"
				<?php
				...
				?>
				class="view-tour-btn"><?php echo esc_html( $settings['button_text'] ); ?></a>
			</div>
		</div>

		<?php
		if ( 'yes' === $settings['display_whatsapp_share'] ) {
			?>
